...
def cook_refs(refs, n=4):
    ...
    # refs = [ref.split() for ref in refs] # TODO
    maxcounts = {}
    for ref in refs:
...
def computeMaps(predictionfile, goldfile):
  ...
  for row in pf:
    ...
    if len(cols) == 1:
      ...
      (rid, pred) = (cols[0], cols[1])
    if rid not in predictionMap:
      predictionMap[rid] = []
    predictionMap[rid].append(pred.strip())
...
#m1 is the reference map
#m2 is the prediction map
def topNBleuFromMaps(m1, m2, topn_list=[1, 5, 10]):
