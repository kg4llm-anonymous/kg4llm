...
def main():
    ...
    ## Other parameters
    parser.add_argument("--train_filename", default="./filtered_train.top5.ver9.no_rt.jsonl", type=str, 
                        help="The train filename. Should contain the .jsonl files for this task.")
    parser.add_argument("--dev_filename", default="./test.top1.ver9.no_rt.jsonl", type=str, 
                        help="The dev filename. Should contain the .jsonl files for this task.")
    parser.add_argument("--test_filename", default="./test.top1.ver9.no_rt.jsonl", type=str, 
    ...
    if args.do_train:
        ...
        train_features, train_examples = convert_examples_to_features(train_examples, tokenizer,args,stage='train')
        ...
        for step in bar:
            ...
            if args.do_eval and ((global_step + 1) %args.eval_steps == 0) and eval_flag:
                ...
                if 'dev_loss' in dev_dataset:
                    ...
                    eval_features, eval_examples = convert_examples_to_features(eval_examples, tokenizer, args,stage='dev')
                ...
                output_model_file = os.path.join(last_output_dir, f"pytorch_model.{global_step+1}.bin")
    ...
    if args.do_test:
        if args.test_filename is not None:
            ...
            eval_features, eval_examples = convert_examples_to_features(eval_examples, tokenizer, args,stage='test')
            all_source_ids = torch.tensor([f.source_ids for f in eval_features], dtype=torch.long)
            all_source_mask = torch.tensor([f.source_mask for f in eval_features], dtype=torch.long)    
            ...
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                ...
                with torch.no_grad():
                    ...
                    for pred in preds:
                        ...
                        p.append(text)
            model.train()
            logger.info("Save file: {}".format(os.path.join(args.output_dir,"test_{}.output".format(idx))))
            logger.info("Save file: {}".format(os.path.join(args.output_dir,"test.gold")))
            with open(os.path.join(args.output_dir,"test_{}.output".format(str(idx))),'w') as f, open(os.path.join(args.output_dir,"test.gold"),'w') as f1:
                for refs, gold in zip(p, eval_examples):
                    for ref in refs:
                        f.write(str(gold.idx)+'\t'+ref+'\n')
                    f1.write(str(gold.idx)+'\t'+gold.target+'\n') 
            (goldMap, predictionMap) = bleu.computeMaps(os.path.join(args.output_dir, "test_{}.output".format(idx)), os.path.join(args.output_dir, "test.gold")) 
            
            metrics(chr, eval_examples, goldMap, predictionMap)
